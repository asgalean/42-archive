...
void floodfill(t_info *info, int row, int col)
{
	...
	if (row >= 0 && info->map[row - 1][col] != '1' && info->ff_map[row - 1][col] != 'X')
		floodfill(info, row - 1, col);
	if (col >= 0 && info->map[row][col - 1] != '1' && info->ff_map[row][col - 1] != 'X')
...
void	map_to_window(void	*mlx, void *mlx_win,  t_info info, t_img img)
{
	...
	while (i < info.row_max)
	{
		...
		{
			if (info.map[i][j] == '0')
				mlx_put_image_to_window(mlx, mlx_win, img.floor, j * 64, i * 64);
			if (info.map[i][j] == '1')
				mlx_put_image_to_window(mlx, mlx_win, img.wall, j * 64, i * 64);
			if (info.map[i][j] == 'E')
				mlx_put_image_to_window(mlx, mlx_win, img.exit_0, j * 64, i * 64);
			if (info.map[i][j] == 'C')
				mlx_put_image_to_window(mlx, mlx_win, img.coin, j * 64, i * 64);
			if (info.map[i][j] == 'P')
				mlx_put_image_to_window(mlx, mlx_win, img.p_idle, j * 64, i * 64);
			j++;
		}
		i++;
	}
...
{
	mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.floor, info->player_col * 64, info->player_row * 64);
}
...
void	print_exit(t_info *info, t_window *mlx, t_img img)
{
	if ((info->owned_tokens * 100)/info->tokens < 25)
		mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.exit_0, info->exit_col * 64, info->exit_row * 64);
	else if ((info->owned_tokens * 100)/info->tokens < 50)
		mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.exit_25, info->exit_col * 64, info->exit_row * 64);
	else if ((info->owned_tokens * 100)/info->tokens < 75)
		mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.exit_50, info->exit_col * 64, info->exit_row * 64);
	else if ((info->owned_tokens * 100)/info->tokens < 100)
		mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.exit_75, info->exit_col * 64, info->exit_row * 64);
	else if ((info->owned_tokens * 100)/info->tokens == 100)
		mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.exit_100, info->exit_col * 64, info->exit_row * 64);
}
...
{
	mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.p_left, info->player_col * 64, info->player_row * 64);
}
...
{
	mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.p_right, info->player_col * 64, info->player_row * 64);
}
...
{
	if (info->moving == 1)
		mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.p_up_1, info->player_col * 64, info->player_row * 64);
	if (info->moving == 2)
		mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.p_up_2, info->player_col * 64, info->player_row * 64);
}
...
{
	if (info->moving == 1)
		mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.p_down_1, info->player_col * 64, info->player_row * 64);
	if (info->moving == 2)
		mlx_put_image_to_window(mlx->mlx, mlx->mlx_win, img.p_down_2, info->player_col * 64, info->player_row * 64);
}
...
void	free_images(t_window *mlx)
{
	...
	mlx_destroy_image(mlx->mlx, mlx->img.p_left);
	mlx_destroy_image(mlx->mlx, mlx->img.p_right);
...
void	move_left(t_info *info, t_window *mlx)
{
	...
	if (info->player_col > 0 && info->map[info->player_row][info->player_col - 1] != '1')
	{
		info->on_exit = 0;
...
void	move_right(t_info *info, t_window *mlx)
{
	...
	if (info->player_col < info->col_max && info->map[info->player_row][info->player_col + 1] != '1')
	{
		info->on_exit = 0;
		if (info->map[info->player_row][info->player_col + 1] == 'C')
		{
...
void	move_up(t_info *info, t_window *mlx)
{
	...
	if (info->player_row > 0 && info->map[info->player_row - 1][info->player_col] != '1')
	{
		info->on_exit = 0;
...
void	move_down(t_info *info, t_window *mlx)
{
	...
	if (info->player_row < info->row_max && info->map[info->player_row + 1][info->player_col] != '1')
	{
		info->on_exit = 0;
		if (info->map[info->player_row + 1][info->player_col] == 'C')
		{
...
int	main(int argc, char *argv[])
{
	...
	if (!extension_valid(argv[1]))
		return (1);
	if(!row_counter(argv[1], &mlx.info))
		return (1);
	...
	mlx.mlx = mlx_init();
	mlx.mlx_win = mlx_new_window(mlx.mlx, mlx.info.col_max * 64, mlx.info.row_max * 64, "Pokemon Silver");
